...
-- End wrapper with a valid customer query select matching function signature
SELECT id, name, email, phone, city, total_spent, order_count, last_order_date, tags, created_at FROM customers WHERE 1=0
  `.trim();

  const { data, error } = await supabase.rpc('execute_segment_query', {
    where_clause: sqlStatements
  });

  if (error) {
    console.error('Error executing SQL:', error);
  } else {
    console.log('SQL applied successfully! Table schema updated.');
  }
}
